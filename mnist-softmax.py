...
class SoftMaxClassifier(object):
    def __init__(self, x, n_in, n_out, y):
        prefix = 'sm'
        W_values = uniform_init(n_in, n_out)
        self.W = theano.shared(value=W_values, name=prefix+'_W', borrow=True)
        b_values = np.zeros((n_out,), dtype=floatX)
        self.b = theano.shared(value=b_values, name=prefix+'_b', borrow=True)

        self.out = T.nnet.softmax(T.dot(x, self.W) + self.b)
...
class SoftMax(object):
    def __init__(self, n_in, n_out, optimizer):

        self.x = T.matrix('x', dtype=floatX)
        self.y = T.vector('y', dtype='int64')  # one-hot vector
        lr = T.scalar(dtype=floatX)

        self.sm = SoftMaxClassifier(self.x, n_in, n_out, y=self.y)
        self.cost = self.sm.cost / self.x.shape[0]
        self.params = self.sm.params
        ...
        self.test = theano.function(
            ...
            outputs=[self.cost, self.sm.y_pred],
            updates=None
        )
...
def main():
    ...
    print('Best perf: %f (data_frac=%f, lr=%f)' % (meanBestPerf, args.data_frac, args.lr))
